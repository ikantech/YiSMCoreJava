...
public class RSACipherTest {
    ...
    private String[] rsaNoPaddingCipherDatas = new String[] {
            "",
    ...
    public void testGenKey() {
        try {
            //生成密钥对
            YiCryptoKey cryptoKey = YiCryptoKey.genRSAKeyPair(0x03, 1024);
            ...
            //这里可以将密钥对保存到本地
            System.out.println("PKCS#1 PrivateKey:\n" + pkcs1PrivStr);
            System.out.println("PKCS#8 PrivateKey:\n" + pkcs8PrivStr);

            System.out.println("\nPKCS#1 PublicKey:\n" + pkcs1PubStr);
            System.out.println("PKCS#8 PublicKey:\n" + pkcs8PubStr);

            System.out.println("\nn: " + YiSMCore.toHexString(cryptoKey.getRSA_NBytes()));
            System.out.println("e: " + YiSMCore.toHexString(cryptoKey.getRSA_EBytes()));
            System.out.println("d: " + YiSMCore.toHexString(cryptoKey.getRSA_DBytes()));
            System.out.println("p: " + YiSMCore.toHexString(cryptoKey.getRSA_PBytes()));
            System.out.println("q: " + YiSMCore.toHexString(cryptoKey.getRSA_QBytes()));
            System.out.println("dp: " + YiSMCore.toHexString(cryptoKey.getRSA_DPBytes()));
            System.out.println("dq: " + YiSMCore.toHexString(cryptoKey.getRSA_DQBytes()));
            System.out.println("qinv: " + YiSMCore.toHexString(cryptoKey.getRSA_QInvBytes()));
        }catch (Exception ex) {
    ...
    public void testCreateCipherNoPadding() {
        try {
            ...
            byte[] cipherBytes = YiSMCore.getInstance("RSA/None/NoPadding")
                    ...
                    .update(bytes, 32, 95)
                    .doFinal();
            System.out.println("privK cipher: " + YiSMCore.toHexString(cipherBytes));

            bytes = YiSMCore.getInstance("RSA/None/NoPadding")
                    .setupForCipher(false, pubKey)
                    .update(cipherBytes, 0, 32)
                    .update(cipherBytes, 32, 96)
                    .doFinal();
            System.out.println("privK plain: " + YiSMCore.toHexString(bytes));

            cipherBytes = YiSMCore.getInstance("RSA/None/NoPadding")
                    .setupForCipher(true, pubKey)
                    .update(bytes, 0, 32)
                    .update(bytes, 32, 95)
                    .doFinal();
            System.out.println("pubK cipher: " + YiSMCore.toHexString(cipherBytes));
            ...
            plainBytes = YiSMCore.getInstance("RSA/None/NoPadding")
                    .setupForCipher(false, privKey)
                    .doFinal(YiSMCore.fromHexString(rsaNoPaddingCipherDatas[1]));
            Assert.assertArrayEquals(plainBytes, bytes);
        }catch (Exception ex) {
            Assert.fail(ex.getMessage());
        }
        ...
        try {
            ...
            for(int i = 0; i < cipherAlgs.length; i++) {
                ...
                byte[] cipherBytes = YiSMCore.getInstance("RSA/None/" + alg)
                        ...
                        .doFinal();
                System.out.println(alg + " privK cipher: " + YiSMCore.toHexString(cipherBytes));
                ...
                bytes = YiSMCore.getInstance("RSA/None/" + alg)
                        ...
                        .doFinal();
                System.out.println(alg + " privK plain: " + YiSMCore.toHexString(bytes));

                cipherBytes = YiSMCore.getInstance("RSA/None/" + alg)
                        .setupForCipher(true, pubKey)
                        .update(bytes, 0, 32)
                        .update(bytes, 32, 95)
                        .update(bytes, 127, bytes.length - 127)
                        .doFinal();
                System.out.println(alg + " pubK cipher: " + YiSMCore.toHexString(cipherBytes));
                ...
                byte[] plainBytes = YiSMCore.getInstance("RSA/None/" + alg)
                        .setupForCipher(false, pubKey)
                        .doFinal(YiSMCore.fromHexString(rsaWithPaddingCipherDatas[i][0]));
                Assert.assertArrayEquals(plainBytes, bytes);

                plainBytes = YiSMCore.getInstance("RSA/None/" + alg)
                        .setupForCipher(false, privKey)
