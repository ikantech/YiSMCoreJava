...
/**
 * Created by ikantech on 20-4-19.
 */
public class RSASignTest {
    ...
    public void testPrivateSign() {
        try {
            ...
            privateKey.setupRSAKeyFromPEM(pkcs1PrivKStr);

            YiCryptoKey publicKey = new YiCryptoKey();
            publicKey.setupRSAKeyFromPEM(pkcs1PubKStr);

            for (int i = 0; i < algs.length; i++) {
                if (algs[i].equals("SHA512WithRSA/PSS") ||
                        algs[i].equals("SHA3-512WithRSA/PSS") ||
                        algs[i].equals("BLAKE-2BWithRSA/PSS")) {
                    try {
                        YiSMCore.getInstance(algs[i])
                                .setupForCipher(true, privateKey);
                ...
                byte[] cipherBytes = YiSMCore.getInstance(algs[i])
                        .setupForSigner(true, privateKey)
                        .update(plainBytes, 0, 13)
                        .update(plainBytes, 13, plainBytes.length - 13)
                        .generateSignature();

                System.out.println(algs[i] + " sign data: " + YiSMCore.toHexString(cipherBytes));

                boolean verifyResult = YiSMCore.getInstance(algs[i])
                        .setupForSigner(false, publicKey)
                        .update(plainBytes, 0, 13)
                        .update(plainBytes, 13, plainBytes.length - 13)
